...
```
Explanation: This query shows both POST and GET requests; visualizing counts side by side will reveal if POST requests outnumber GET requests.
...
dns.question.type:"A"
```
Explanation: This query finds all DNS “A” record lookups; charting them in 1-minute buckets and watching for counts >100 highlights spikes.
...
```lucene
dns.question.name.keyword:/[A-Za-z0-9]{8}\.[A-Za-z]{3}/
```
Explanation: This regex matches domains with eight alphanumeric characters plus a three-letter TLD (e.g., `abcd1234.xyz`), flagging likely DGA domains.
...
```lucene
network.bytes_out:[1 TO *] AND network.bytes_out > network.bytes_in
```
Explanation: This query filters for sessions where the outbound byte count exceeds inbound, indicating possible data exfiltration.
...
Windows Event Log ID 4634 indicates logoff events, which can be useful in tracking user activity and potential unauthorized access. The corresponding Sysmon Event ID for logoff events is Sysmon Event ID 2.
